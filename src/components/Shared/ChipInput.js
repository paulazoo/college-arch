...
class ChipInput extends React.Component {
   ...
   */
  handleAddChip(inputChip) {
    if (this.props.onBeforeAdd && !this.props.onBeforeAdd(inputChip)) {
    ...
    if (inputChip.includes(' ') === true) {
      parsedChip = inputChip.split(' ');
    } else {
      parsedChip = [inputChip];
    }
    parsedChip.forEach((chip) => {
